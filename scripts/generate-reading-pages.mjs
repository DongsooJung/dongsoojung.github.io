...
const NOTES_PATH = path.join(READING_DIR, 'data', 'featured-reading-notes.json');
...
function renderReadingNote(note) {
  const list = (title, values) => values?.length
    ? `<section><h3>${title}</h3><ul>${values.map((value) => `<li>${esc(value)}</li>`).join('')}</ul></section>` : '';
  const related = (note.relatedProjects || []).filter((project) => /^\/[\w\p{L}-]+(?:\/[\w\p{L}-]+)*\/$/u.test(project.url || ''));
  return `<article class="reading-note" id="${esc(note.id)}">
    <p class="eyebrow">대표 독서노트 · ${esc(note.genre)}</p>
    <h2>읽기와 연구를 잇는 질문</h2>
    <p class="lead">${esc(note.oneline)}</p>
    ${list('핵심 관점', note.keyPoints)}
    ${list('연구·교육·사업에 적용', note.application)}
    ${note.tags?.length ? `<div class="keywords">${note.tags.map((tag) => `<span>#${esc(tag)}</span>`).join('')}</div>` : ''}
    ${related.length ? `<section><h3>연결된 연구·전략</h3><div class="project-links">${related.map((project) => `<a href="${esc(project.url)}">${esc(project.label)} <span aria-hidden="true">→</span></a>`).join('')}</div></section>` : ''}
    <p class="source-note">완독 여부·별점·읽은 날짜를 표시하지 않은 편집형 독서노트입니다. 개인 완독 독후감과 구분됩니다.</p>
  </article>`;
}

function renderBookHero(book, reviews, note, priceParts, priceUpdatedAt, purchaseLinks) {
  const coverUrl = safeCoverUrl(book.coverUrl);
  const content = `<p class="eyebrow">${book.featuredAi ? 'AI 기술 대표 도서 · ' : ''}${esc(book.category || '기타')} · ${esc(book.subCategory || '분류 미등록')}</p>
      ...
      <div class="facts">
        ...
        <div><span>독서기록</span><strong>${reviews.length ? `${reviews.length}편` : note ? '대표 노트 1편' : '준비 중'}</strong></div>
      </div>
...
function renderBookPage(book, reviews, note, related, priceUpdatedAt) {
  const displayBook = note?.author && !book.author ? { ...book, author: note.author } : book;
  const canonical = canonicalFor(book.slug);
  const indexable = reviews.length > 0 || Boolean(note) || Boolean(book.featuredAi && book.indexable === true && book.curationNote);
  const description = reviews[0]?.oneline
    || note?.oneline
    || book.curationNote
    || `${book.title}${displayBook.author ? `, ${displayBook.author}` : ''} 도서정보와 StargateEdu 독서기록`;
  const purchaseLinks = safePurchaseLinks(displayBook);
  ...
  const structuredData = makeStructuredData(displayBook, reviews, canonical);
  return `<!doctype html>
<html lang="ko">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <main id="content">
    ...
    ${renderBookHero(displayBook, reviews, note, priceParts, priceUpdatedAt, purchaseLinks)}
...
` : ''}${note ? `    ${renderReadingNote(note)}
` : ''}${reviews.length ? `    ${reviews.map(renderReview).join('')}
` : note ? '' : `    <section class="empty"><h2>독서기록 준비 중</h2><p>도서정보를 먼저 공개했으며, 읽은 이유와 적용점은 정리되는 대로 추가합니다.</p></section>
...
export async function generateReadingPages() {
  const [booksPayload, reviewsPayload, featuredPayload, notesPayload, previousManifest] = await Promise.all([
    ...
    fs.readFile(NOTES_PATH, 'utf8').then(JSON.parse).catch(() => ({ notes: [] })),
  ...
  for (const book of books) {
    ...
    const html = renderBookPage(book, reviews, note, related, booksPayload.priceUpdatedAt);
