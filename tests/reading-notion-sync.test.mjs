...
test('현재 598권 스냅샷을 손실 없이 읽고 기존 표시 필드를 유지한다', async () => {
  ...
  for (const book of payload.books) {
    for (const key of ['title', 'author', 'category', 'subCategory', 'read', 'rating', 'aladinUrl', 'listPrice', 'slug']) {
...
test('내용이 같으면 두 데이터셋의 생성 시각을 보존한다', () => {
  const linked = { books: [{ title: '책', notionId: 'book1', relatedLogIds: [] }], posts: [{ id: 'log', relatedBookIds: [] }] };
  const previousBooks = { generatedAt: 'books-old', catalogStatus: 'connected', books: [{ title: '책', notionId: 'book1', slug: '책-b1' }] };
